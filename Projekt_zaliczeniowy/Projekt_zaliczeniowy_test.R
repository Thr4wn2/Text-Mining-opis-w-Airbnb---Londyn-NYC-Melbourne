...
# Losowa próbka 1000 opisów z każdego miasta (ze względu na rozmiar plików)
set.seed(42)
...
# 2B. STEMMING
# ============================================================
# Stemming sprowadza słowa do ich rdzenia (np. "walking" -> "walk",
# "restaurants" -> "restaur"). Prowadząca zaleciła go wprost w mailu.
# Używamy podejścia z zajęć: stemming + stemCompletion (uzupełnienie rdzeni)
# żeby słowa w wynikach były czytelne, a nie ucięte.

# Zachowaj kopię korpusu jako słownik do uzupełniania rdzeni
corpus_copy <- corpus

# Wykonaj stemming
corpus_stemmed <- tm_map(corpus, stemDocument)

# Funkcja pomocnicza: uzupełnia rdzenie słowo po słowie (z zajęć)
complete_stems <- content_transformer(function(x, dict) {
  x <- unlist(strsplit(x, " "))
  x <- stemCompletion(x, dictionary = dict, type = "longest")
  paste(x, collapse = " ")
})

# Uzupełnij rdzenie korzystając z oryginalnego korpusu jako słownika
corpus_completed <- tm_map(corpus_stemmed, complete_stems, dict = corpus_copy)
corpus_completed <- tm_map(corpus_completed, toSpace, "NA")   # usuń NA po stemCompletion
corpus_completed <- tm_map(corpus_completed, stripWhitespace)

# Wyciągnij wyczyszczone + zestemmowane teksty
# (używane w sekcji klastrowania zamiast surowego cleaned_texts)
stemmed_texts <- sapply(corpus_completed, function(doc) doc$content)
...
# Podejście z zajęć: kmeans() na macierzy DTM, fviz_nbclust, chmury per klaster.

# Budujemy DTM na zestemmowanym korpusie z wagami TF-IDF
...
for (k in c(3, 4)) {
  ...
  # DODANE: dołącz miasto do tabeli (nie mamy nazw plików jak z zajęć,
  # ale mamy wektor data$city – indeksujemy przez zachowane niepuste wiersze)
  ...
  # Rozkład miast w klastrach (DODANE – zastępuje wykres z zajęć
  # który pokazywał nazwy plików, tu nie mamy plików)
  doc_table %>%
